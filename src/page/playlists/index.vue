<template>
    <div class="playlists" ref="playlists">
        <div class="top-play-list-card" v-if="state.topPlaylist.id">
            <TopPlaylistCard :desc="state.topPlaylist.description" :id="state.topPlaylist.id"
        ...
        <div class="playlist-cards">
            <PlaylistCard v-for="item in state.playlists" :key="item.id"
                :desc="`播放量：${$utils.formatNumber(item.playCount)}`" :id="item.id" :img="item.coverImgUrl"
                :name="item.name" />
        </div>
        <Pagination :current-page.sync="state.currentPage" :page-size="PAGE_SIZE" :total="state.total"
            @current-change="onPageChange" class="pagination"></Pagination>
    </div>
</template>
