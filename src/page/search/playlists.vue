<template>
    <div class="search-playlists">
        <WithPagination :getData="getSearch" :getDataParams="searchParams" :limit="50"
            :scrollTarget="searchRoot.$refs && searchRoot.$refs.header" :total="playlistCount"
            @getDataSuccess="onGetPlaylists">
            <div class="list-wrap">
                <PlaylistCard :desc="`播放量：${$utils.formatNumber(item.playCount)}`" :id="item.id" :img="item.coverImgUrl"
                    :key="item.id" :name="item.name" v-for="item in playlists" />
            </div>
        </WithPagination>
    </div>
</template>
...
const onGetPlaylists = ({ result: { playlists: newPlaylists, playlistCount: newPlaylistCount } }) => {
