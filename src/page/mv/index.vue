  ...
  <div class="mv" v-if="$utils.isDef(mvDetail.id)">
    <div class="mv-content">
      <div class="left">
        ...
        <div class="desc">
          <span class="date"
            >发布：{{ $utils.formatDate(mvDetail.publishTime, 'yyyy-MM-dd') }}</span
          >
...
<script setup>
  ...
  const init = async () => {
    const [{ data: mvDetailData }, { data: mvPlayInfoData }, { mvs: simiMvsData }] =
      await Promise.all([getMvDetail(props.id), getMvUrl(props.id), getSimiMv(props.id)]);
